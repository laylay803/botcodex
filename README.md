...
- Важно: команды из Discord работают только пока процесс бота запущен. Если вы полностью остановили процесс в VS Code, бот физически не может получить `@start` из Discord до повторного запуска процесса (или запуска через pm2/systemd).
